...
- name: Liwei Wang
  orcid: null
  s2_author_id: '39060743'
  s2_url: null
published_date: Sep 7, 2020
published_date_iso: '2020-09-07'
published_venue: ICML 2020
published_conference: ICML 2020
published_conference_short: ICML
published_conference_slug: icml
abstract: Normalization plays an important role in the optimization of deep neural
  networks. While there are standard normalization methods in computer vision and
  natural language processing, there is limited understanding of how to effectively
  normalize neural networks for graph representation learning. In this paper, we propose
  a principled normalization method, Graph Normalization (GraphNorm), where the key
  idea is to normalize the feature values across all nodes for each individual graph
  with a learnable shift. Theoretically, we show that GraphNorm serves as a preconditioner
  that smooths the distribution of the graph aggregation's spectrum, leading to faster
  optimization. Such an improvement cannot be well obtained if we use currently popular
  normalization methods, such as BatchNorm, which normalizes the nodes in a batch
  rather than in individual graphs, due to heavy batch noises. Moreover, we show that
  for some highly regular graphs, the mean of the feature values contains graph structural
  information, and directly subtracting the mean may lead to an expressiveness degradation.
  The learnable shift in GraphNorm enables the model to learn to avoid such degradation
  for those cases. Empirically, Graph neural networks (GNNs) with GraphNorm converge
...
- &id003
  ...
  - model: GIN + BatchNorm
    ...
    is_std_outlier: false
  - model: GIN+LayerNorm
    model_key: gin+layernorm
    model_plain: GIN+LayerNorm
    value: 0.801
    std: 0.008
    paper_value: 0.801
    paper_std: 0.008
...
- &id001
  ...
  - model: GIN+GraphNorm
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: Test accuracy on MUTAG graph classification
    date: Sep 7, 2020
    date_display: Sep 2020
    date_iso: '2020-09-07'
    published_venue: ICML 2020
...
- &id002
  ...
  - model: WL subtree
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: Test accuracy on NCI1 graph classification
    date: Sep 7, 2020
    date_display: Sep 2020
    date_iso: '2020-09-07'
    published_venue: ICML 2020
...
- &id004
  ...
  - model: GraphNorm
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: Test accuracy on RDT-B graph classification
    date: Sep 7, 2020
    date_display: Sep 2020
    date_iso: '2020-09-07'
    published_venue: ICML 2020
...
- &id005
  ...
  - model: GCN+GraphNorm
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: Official setting from hu2020open, 10 random seeds
    date: Sep 7, 2020
    date_display: Sep 2020
    date_iso: '2020-09-07'
    published_venue: ICML 2020
    ...
    is_best: false
    is_std_outlier: false
  - model: GCN+InstanceNorm
    model_key: gcn+instancenorm
    model_plain: GCN+InstanceNorm
    value: 0.7818
    std: 0.0042
    paper_value: 0.7818
    paper_std: 0.0042
  ...
  - model: GCN
    ...
    is_std_outlier: false
  - model: GCN+BatchNorm
    model_key: gcn+batchnorm
    model_plain: GCN+BatchNorm
    value: 0.7622
    std: 0.0095
    paper_value: 0.7622
    paper_std: 0.0095
    ...
    is_std_outlier: false
  - model: GCN+LayerNorm
    model_key: gcn+layernorm
    model_plain: GCN+LayerNorm
    value: 0.7504
    std: 0.0048
    paper_value: 0.7504
    paper_std: 0.0048
...
main_figure: /figures/2009.03294/main_figure.jpegoptim.jpg
---
