...
abstract: Despite the enormous success of Graph Convolutional Networks (GCNs) in modeling
  graph-structured data, most of the current GCNs are shallow due to the notoriously
  challenging problems of over-smoothening and information squashing along with conventional
  difficulty caused by vanishing gradients and over-fitting. Previous works have been
  primarily focused on the study of over-smoothening and over-squashing phenomena
  in training deep GCNs. Surprisingly, in comparison with CNNs/RNNs, very limited
  attention has been given to understanding how healthy gradient flow can benefit
  the trainability of deep GCNs. In this paper, firstly, we provide a new perspective
  of gradient flow to understand the substandard performance of deep GCNs and hypothesize
  that by facilitating healthy gradient flow, we can significantly improve their trainability,
  as well as achieve state-of-the-art (SOTA) level performance from vanilla-GCNs.
  Next, we argue that blindly adopting the Glorot initialization for GCNs is not optimal,
  and derive a topology-aware isometric initialization scheme for vanilla-GCNs based
  on the principles of isometry. Additionally, contrary to ad-hoc addition of skip-connections,
  we propose to use gradient-guided dynamic rewiring of vanilla-GCNs with skip connections.
  Our dynamic rewiring method uses the gradient flow within each layer during training
  to introduce on-demand skip-connections adaptively. We provide extensive empirical
  evidence across multiple datasets that our methods improve gradient flow in deep
  vanilla-GCNs and significantly boost their performance to comfortably compete and
  outperform many fancy state-of-the-art methods. Codes are available at:.
codebase_url: https://github.com/VITA-Group/GradientGCN
extraction_model: cyankiwi/gemma-4-26B-A4B-it-AWQ-4bit
has_results: true
paper_type: method
proposed_models:
- topology-aware isometric initialization and Gradient-guided rewiring
- topology-aware initialization and gradient-guided rewiring
mrr: 0.0021
adjusted_mrr: 0.0021
mrr_dataset_count: 3
...
- &id003
  ...
  - model: GCN
    ...
    feature_source_evidence: ''
    table_ref: Table 1
    source_ref: Kipf2017SemiSupervisedCW
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Planetoid split (Yang et al. 2016 fixed split) as per Chen et al.
      2021 protocol.
    date: Oct 14, 2022
    ...
    is_std_outlier: false
  - model: GCN + Our Init
    model_key: gcn + our init
    model_plain: GCN + Our Init
    value: 0.782
    std: null
    paper_value: 0.782
    paper_std: null
    ...
    true_value: 0.782
    true_std: null
    value_gap_source_arxiv: '2210.08122'
    value_gap_source_title: 'Old can be Gold: Better Gradient Flow can Make Vanilla-GCNs
      Great Again'
...
- &id001
  ...
  - model: SGC
    ...
    is_best: false
    is_std_outlier: false
  - model: topology-aware isometric initialization and Gradient-guided rewiring
    model_key: topology-aware isometric initialization and gradient-guided rewiring
    model_plain: topology-aware isometric initialization and Gradient-guided rewiring
    value: 0.831
    std: 0.0025
    paper_value: 0.831
    paper_std: 0.0025
    ...
    is_std_outlier: false
  - model: InceptionGCN
    model_key: inceptiongcn
    model_plain: InceptionGCN
    value: 0.817
    std: null
    paper_value: 0.817
    paper_std: null
...
- &id005
  ...
  - model: APPNP
    ...
    feature_source_evidence: ''
    table_ref: Table 1
    source_ref: Klicpera2019PredictTP
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
    ...
    gap_vs_at_pub: 0.33089999999999997
    worse_than_at_pub: true
    surpassed_since_pub: false
    better_than_at_pub: false
    insignificant_improvement_at_pub: false
...
- &id002
  ...
  - model: GCN
    ...
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Planetoid split (Yang et al. 2016 fixed split) as per setup prose.
    date: Oct 14, 2022
    date_display: Oct 2022
    date_iso: '2022-10-14'
    published_venue: NeurIPS 2022
  ...
  - model: InceptionGCN
    ...
    feature_source_evidence: ''
    table_ref: Table 1
    source_ref: kazi2019inceptiongcn
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Planetoid split (Yang et al. 2016 fixed split) as per setup prose.
...
- &id004
  ...
  - model: GAT
    ...
    value_gap_source_date_label: '2025'
    gap_vs_at_pub: 0.6826000000000001
    worse_than_at_pub: true
    surpassed_since_pub: false
    better_than_at_pub: false
    insignificant_improvement_at_pub: false
  ...
  - model: JKNet
    ...
    is_best: false
    is_std_outlier: false
  - model: topology-aware initialization and gradient-guided rewiring
    model_key: topology-aware initialization and gradient-guided rewiring
    model_plain: topology-aware initialization and gradient-guided rewiring
    value: 0.6428
    std: 0.0293
    paper_value: 0.6428
    paper_std: 0.0293
...
main_figure: /figures/2210.08122/main_figure.jpegoptim.jpg
---
