...
- name: Corentin Tallec
  orcid: null
  s2_author_id: '31803582'
  s2_url: null
- name: M. G. Azar
  orcid: null
  s2_author_id: '37666967'
  s2_url: null
- name: Mehdi Azabou
  orcid: null
...
- name: Eva L. Dyer
  orcid: null
  s2_author_id: '1746363'
  s2_url: null
- name: R. Munos
  orcid: null
  s2_author_id: '1708654'
  s2_url: null
...
- name: Michal Valko
  orcid: null
  s2_author_id: '1806291'
  s2_url: null
published_date: Feb 12, 2021
published_date_iso: '2021-02-12'
...
abstract: Self-supervised learning provides a promising path towards eliminating the
  need for costly label information in representation learning on graphs. However,
  to achieve state-of-the-art performance, methods often need large numbers of negative
  examples and rely on complex augmentations. This can be prohibitively expensive,
  especially for large graphs. To address these challenges, we introduce Bootstrapped
  Graph Latents (BGRL) - a graph representation learning method that learns by predicting
  alternative augmentations of the input. BGRL uses only simple augmentations and
  alleviates the need for contrasting with negative examples, and is thus scalable
  by design. BGRL outperforms or matches prior methods on several established benchmarks,
  while achieving a 2-10x reduction in memory costs. Furthermore, we show that BGRL
  can be scaled up to extremely large graphs with hundreds of millions of nodes in
  the semi-supervised regime - achieving state-of-the-art performance and improving
  over supervised baselines where representations are shaped only through label information.
  In particular, our solution centered on BGRL constituted one of the winning entries
  to the Open Graph Benchmark - Large Scale Challenge at KDD Cup 2021, on a graph
  orders of magnitudes larger than all previously available benchmarks, thus demonstrating
  the scalability and effectiveness of our approach.
codebase_url: https://github.com/nerdslab/bgrl
extraction_model: cyankiwi/gemma-4-26B-A4B-it-AWQ-4bit
has_results: true
paper_type: method
proposed_models:
- BGRL
- BGRL MeanPooling encoder
- BGRL GAT encoder
mrr: 0.01
adjusted_mrr: 0.0033
mrr_dataset_count: 1
...
- &id001
  ...
  - model: BGRL GAT encoder
    ...
    protocol_note: Micro-F1 on PPI using linear evaluation on frozen embeddings, averaged
      over 20 random model initializations.
    date: Feb 12, 2021
  ...
  - model: GRACE GAT encoder
    ...
    is_best: false
    is_std_outlier: false
  - model: GRACE MeanPooling encoder
    model_key: grace meanpooling encoder
    model_plain: GRACE MeanPooling encoder
    value: 0.6966
    std: 0.0015
    paper_value: 0.6966
    paper_std: 0.0015
    ...
    is_best: false
    is_std_outlier: false
  - model: BGRL MeanPooling encoder
    model_key: bgrl meanpooling encoder
    model_plain: BGRL MeanPooling encoder
    value: 0.6941
    std: 0.0015
    paper_value: 0.6941
    paper_std: 0.0015
...
- &id002
  ...
  - model: GRACE
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: GRACE is a contrastive method
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Accuracy on WikiCS using frozen linear evaluation (logistic regression)
      on embeddings.
    date: Feb 12, 2021
    date_display: Feb 2021
    date_iso: '2021-02-12'
  ...
  - model: BGRL
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: BGRL uses simple graph augmentations
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: Random-Init
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: Random initialization
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: DGI
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: DGI is a GNN-based contrastive method
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: GCA
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: GCA is a contrastive method
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: MVRL
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: MVRL is a GNN-based contrastive method
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: DeepWalk
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: DeepWalk is a walk-based method
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: DeepWalk + feat.
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: DeepWalk + feat.
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: Supervised GCN
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: Supervised GCN uses standard features
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: GMI
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: GMI is a GNN-based contrastive method
    table_ref: Table 1
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
...
- scope: node-level
  ...
  - benchmark: Classic
    ...
    - dataset: WikiCS
      dataset_slug: wikics
main_figure: /figures/2102.06514/main_figure.jpegoptim.jpg
---
