...
source_url: ''
authors:
- name: Jan Tönshoff Martin Ritzert Eran Rosenbluth Martin Grohe
  orcid: null
...
abstract: The recent Long-Range Graph Benchmark (LRGB, Dwivedi et al. 2022) introduced
  a set of graph learning tasks strongly dependent on long-range interaction between
  vertices. Empirical evidence suggests that on these tasks Graph Transformers significantly
  outperform Message Passing GNNs (MPGNNs). In this paper, we carefully reevaluate
  multiple MPGNN baselines as well as the Graph Transformer GPS (Ramp\'a\v{s}ek et
  al. 2022) on LRGB. Through a rigorous empirical analysis, we demonstrate that the
  reported performance gap is overestimated due to suboptimal hyperparameter choices.
  It is noteworthy that across multiple datasets the performance gap completely vanishes
  after basic hyperparameter optimization. In addition, we discuss the impact of lacking
  feature normalization for LRGB's vision datasets and highlight a spurious implementation
  of LRGB's link prediction metric. The principal aim of our paper is to establish
  a higher standard of empirical rigor within the graph machine learning community.
...
- &id004
  ...
  - model: GPS
    ...
    input_feature_source: raw_features
    feature_source_evidence: The paper notes that the 'Ours' section (which includes
      GPS) uses feature normalization, whereas the LRGB baseline GPS (from the first
      part of Table 4/subtable) does not.
    table_ref: Table 4
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Test F1 on COCO-SP split
    date: Sep 1, 2023
  ...
  - model: GatedGCN
    ...
    input_feature_source: raw_features
    feature_source_evidence: The paper notes that the 'Ours' section (which includes
      GatedGCN) uses feature normalization, whereas the LRGB baseline GatedGCN (from
      the first part of Table 4/subtable) does not.
    table_ref: Table 4
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
  ...
  - model: GINE
    ...
    input_feature_source: raw_features
    feature_source_evidence: The paper notes that the 'Ours' section (which includes
      GINE) uses feature normalization, whereas the LRGB baseline GINE (from the first
      part of Table 4/subtable) does not.
    table_ref: Table 4
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
  ...
  - model: GCN
    ...
    input_feature_source: raw_features
    feature_source_evidence: The paper notes that the 'Ours' section (which includes
      GCN) uses feature normalization, whereas the LRGB baseline GCN (from the first
      part of Table 4/subtable) does not.
    table_ref: Table 4
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
...
- &id003
  ...
  - model: CRaWl
    ...
    feature_source_evidence: Text states CRaWl is the only reported result with normalized
      features.
    table_ref: Table 3
    source_ref: Crawl
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Test F1 on standard split.
    date: Sep 1, 2023
    date_display: Sep 2023
    date_iso: '2023-09-01'
    published_venue: TMLR 2023
  ...
  - model: GPS
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: 'Table 3 (subtable 2) / Table 1: ''Ours'' section uses
    ...
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Test F1 on standard split using tuned hyperparameters and normalization.
    date: Sep 1, 2023
    date_display: Sep 2023
    date_iso: '2023-09-01'
    published_venue: TMLR 2023
  ...
  - model: GCN
    ...
    gap_vs_at_pub: 0.01770000000000002
    worse_than_at_pub: false
    surpassed_since_pub: true
    better_than_at_pub: true
    insignificant_improvement_at_pub: false
...
- &id001
  ...
  - model: GCN
    ...
    protocol_decision: standard
    protocol_note: Test split, MAE metric
    date: Sep 1, 2023
    date_display: Sep 2023
    date_iso: '2023-09-01'
    published_venue: TMLR 2023
    ...
    value_gap_source_date_iso: '2026-05-12'
    value_gap_source_date_label: '2026'
    gap_vs_at_pub: 0.002799999999999997
    worse_than_at_pub: false
    surpassed_since_pub: true
    better_than_at_pub: false
    insignificant_improvement_at_pub: true
  ...
  - model: GatedGCN
    ...
    value_gap_source_date_iso: '2026-05-12'
    value_gap_source_date_label: '2026'
    gap_vs_at_pub: 0.004600000000000021
    worse_than_at_pub: false
    surpassed_since_pub: true
    better_than_at_pub: true
    insignificant_improvement_at_pub: false
  ...
  - model: GINE
    ...
    value_gap_source_date_iso: '2026-05-12'
    value_gap_source_date_label: '2026'
    gap_vs_at_pub: 0.011100000000000027
    worse_than_at_pub: false
    surpassed_since_pub: false
    better_than_at_pub: true
    insignificant_improvement_at_pub: false
...
main_figure: /figures/2309.00367/main_figure.jpegoptim.jpg
---
