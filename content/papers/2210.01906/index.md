...
abstract: 'Understanding generalization and robustness of machine learning models
  fundamentally relies on assuming an appropriate metric on the data space. Identifying
  ...
  problem, TMD reflects the local distribution of node attributes as well as the distribution
  of local computation trees, which are known to be decisive for the learning behavior
  of graph neural networks (GNNs). First, we show that TMD captures properties relevant
  to graph classification: a simple TMD-SVM performs competitively with standard GNNs.
  Second, we relate TMD to generalization of GNNs under distribution shifts, and show
  that it correlates well with performance drop under such shifts. The code is available
  at.'
codebase_url: https://github.com/chingyaoc/TMD
...
- &id001
  ...
  - model: GIN
    ...
    feature_source_evidence: The performances of the baselines are taken from the
      original papers.
    table_ref: Table 1
    ...
    protocol_note: 10-fold CV (90%/10% train-test split) for graph classification
      on MUTAG.
    date: Oct 4, 2022
  ...
  - model: TMD L=4
    ...
    uses_external_data: 0
    input_feature_source: raw_features
    feature_source_evidence: TMD is a pseudometric on attributed graphs... considers
      both the tree structure and local distribution of attributes.
    table_ref: Table 1
  ...
  - model: WWL
    ...
    feature_source_evidence: The performances of the baselines are taken from the
      original papers.
    table_ref: Table 1
    source_ref: togninalli2019wasserstein
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: 10-fold CV (90%/10% train-test split) for graph classification
  ...
  - model: R&G
    ...
    feature_source_evidence: The performances of the baselines are taken from the
      original papers.
    table_ref: Table 1
    source_ref: ramon2003expressivity
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: 10-fold CV (90%/10% train-test split) for graph classification
