...
- name: Xia Hu
  ...
  s2_url: null
published_date: Oct 19, 2022
published_date_iso: '2022-10-19'
...
abstract: Training graph neural networks (GNNs) is extremely time-consuming because
  ...
  dense matrices, sparse matrices are stored in an irregular data format such that
  each row/column may have a different number of non-zero entries. Thus, compared
  to the dense counterpart, approximating sparse operations has two unique challenges
  ...
  the computation resource to different sparse operations, while limiting the total
  used resource below a certain budget. For the second challenge, we cache previously
  sampled sparse matrices to reduce the epoch-wise sampling overhead. To this end,
  we propose Randomized Sparse Computation. In practice, can achieve up to $11.6 $
  speedup for a single sparse operation and $1.6 $ end-to-end wall-clock time speedup
  with almost no accuracy drop. Codes are available at.
codebase_url: https://github.com/warai-0toko/RSC-ICML
...
- &id001
  ...
  - model: GCNII
    ...
    is_best: false
    is_std_outlier: false
  - model: GraphSAGE (full batch)
    model_key: graphsage (full batch)
    model_plain: GraphSAGE (full batch)
    value: 0.9661
    std: 0.0005
    paper_value: 0.9661
    paper_std: 0.0005
  ...
  - model: GCN
    ...
    is_best: false
    is_std_outlier: false
  - model: GraphSAINT + RSC
    model_key: graphsaint + rsc
    model_plain: GraphSAINT + RSC
    value: 0.9624
    std: 0.0003
    paper_value: 0.9624
    paper_std: 0.0003
...
- scope: node-level
  ...
  - benchmark: Classic
    ...
    - dataset: Reddit
      dataset_slug: reddit
main_figure: /figures/2210.10737/main_figure.jpegoptim.jpg
---
