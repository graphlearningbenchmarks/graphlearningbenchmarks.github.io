...
- name: Zhiqiang He
  orcid: null
  s2_author_id: '46915878'
  s2_url: null
published_date: Dec 29, 2021
published_date_iso: '2021-12-29'
...
abstract: In recent years, Graph Neural Networks (GNNs) have shown superior performance
  on diverse real-world applications. To improve the model capacity, besides designing
  aggregation operations, GNN topology design is also very important. In general,
  there are two mainstream GNN topology design manners. The first one is to stack
  aggregation operations to obtain the higher-level features but easily got performance
  drop as the network goes deeper. Secondly, the multiple aggregation operations are
  utilized in each layer which provides adequate and independent feature extraction
  stage on local neighbors while are costly to obtain the higher-level information.
  To enjoy the benefits while alleviating the corresponding deficiencies of these
  two manners, we learn to design the topology of GNNs in a novel feature fusion perspective
  which is dubbed F$^2$GNN. To be specific, we provide a feature fusion perspective
  in designing GNN topology and propose a novel framework to unify the existing topology
  designs with feature selection and fusion strategies. Then we develop a neural architecture
  search method on top of the unified framework which contains a set of selection
  and fusion operations in the search space and an improved differentiable search
  algorithm. The performance gains on diverse datasets, five homophily and three heterophily
  ones, demonstrate the effectiveness of F$^2$GNN. We further conduct experiments
  to show that F$^2$GNN can improve the model capacity while alleviating the deficiencies
  of existing GNN topology design manners, especially alleviating the over-smoothing
  problem, by utilizing different levels of features adaptively..
...
- &id006
  ...
  - model: MLP
    ...
    protocol_note: 10 random splits (48%/32%/20% nodes per class) as per implementation
      details.
    date: Dec 29, 2021
    ...
    gap_vs_at_pub: 0.004100000000000048
  ...
  - model: GraphSAGE
    ...
    input_feature_source: null
    feature_source_evidence: ''
    table_ref: Table tb-performance-heterophily
    source_ref: zhu2020beyond
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
...
- &id002
  ...
  - model: F^2GAT
    ...
    input_feature_source: null
    feature_source_evidence: ''
    table_ref: Table tb-performance-agg
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: 60/20/20 split; average test accuracy over 10 splits
    date: Dec 29, 2021
    date_display: Dec 2021
    date_iso: '2021-12-29'
    published_venue: WWW 2021
  ...
  - model: RES (L4)
    ...
    is_std_outlier: false
  - model: Stacking (L2)
    model_key: stacking (l2)
    model_plain: Stacking (L2)
    value: 0.9149
    std: 0.0021
    paper_value: 0.9149
    paper_std: 0.0021
...
- &id007
  ...
  - model: GCN
    ...
    gap_vs_at_pub: 0.07780000000000009
    worse_than_at_pub: true
    surpassed_since_pub: false
    better_than_at_pub: false
    insignificant_improvement_at_pub: false
...
- &id001
  ...
  - model: F^2GAT
    ...
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: 60% train, 20% val, 20% test split; average of 10 splits
    date: Dec 29, 2021
    date_display: Dec 2021
    date_iso: '2021-12-29'
    published_venue: WWW 2021
  ...
  - model: SANE (L4)
    ...
    is_std_outlier: false
  - model: Stacking (L4)
    model_key: stacking (l4)
    model_plain: Stacking (L4)
    value: 0.8616
    std: 0.0055
    paper_value: 0.8616
    paper_std: 0.0055
...
- &id004
  ...
  - model: MLP
    ...
    evaluation_task: node_classification
    protocol_decision: standard
    protocol_note: Standard split for Texas heterophily task
    date: Dec 29, 2021
    date_display: Dec 2021
    date_iso: '2021-12-29'
    published_venue: WWW 2021
  ...
  - model: Geom-GCN
    ...
    value_gap_source_date_label: TMLR 2025
    gap_vs_at_pub: 0.022300000000000098
    worse_than_at_pub: true
    surpassed_since_pub: false
    better_than_at_pub: false
    insignificant_improvement_at_pub: false
...
- scope: node-level
  ...
  - benchmark: Heterogeneous Graph Benchmarks
    ...
    - dataset: DBLP
      dataset_slug: dblp
single_proposed_model: F^2GNN
main_figure: /figures/2112.14531/main_figure.jpegoptim.jpg
---
