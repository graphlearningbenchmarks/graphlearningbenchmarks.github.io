...
abstract: Graph Neural Networks (GNNs) have emerged as the predominant paradigm for
  ...
  be a key indicator of a model's quality throughout its training phase. To this end,
  we introduce xAI-Drop, a novel topological-level dropping regularizer that leverages
  explainability to pinpoint noisy network elements to be excluded from the GNN propagation
  mechanism. An empirical evaluation on diverse real-world datasets demonstrates that
  our method outperforms current state-of-the-art dropping approaches in accuracy,
...
- &id001
  ...
  - model: DropEdge
    ...
    is_std_outlier: false
  - model: xAI-DropNode
    model_key: xai-dropnode
    model_plain: xAI-DropNode
    value: 0.828
    std: 0.005
    paper_value: 0.828
    paper_std: 0.005
