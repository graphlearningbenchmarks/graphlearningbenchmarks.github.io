...
- name: Jiarong Xu
  orcid: null
  s2_author_id: '2241538357'
  s2_url: null
- name: Renhong Huang
  orcid: null
  s2_author_id: '2264613405'
  s2_url: null
- name: Xin Jiang
  orcid: null
  s2_author_id: '2261899728'
  s2_url: null
- name: Yuxuan Cao
  orcid: null
  s2_author_id: '2265364090'
  s2_url: null
- name: Carl Yang
  orcid: null
  s2_author_id: '2264903811'
  s2_url: null
- name: Chunping Wang
  orcid: null
...
abstract: 'Pre-training on graph neural networks (GNNs) aims to learn transferable
  knowledge for downstream tasks with unlabeled data, and it has recently become an
  active research area. The success of graph pre-training models is often attributed
  to the massive amount of input data. In this paper, however, we identify the curse
  of big data phenomenon in graph pre-training: more training data do not necessarily
  lead to better downstream performance. Motivated by this observation, we propose
  a better-with-less framework for graph pre-training: fewer, but carefully chosen
  data are fed into a GNN model to enhance pre-training. The proposed pre-training
  pipeline is called the data-active graph pre-training (APT) framework, and is composed
  of a graph selector and a pre-training model. The graph selector chooses the most
  representative and instructive data points based on the inherent properties of graphs
  as well as predictive uncertainty. The proposed predictive uncertainty, as feedback
  from the pre-training model, measures the confidence level of the model in the data.
  When fed with the chosen data, on the other hand, the pre-training model grasps
  an initial understanding of the new, unseen data, and at the same time attempts
  to remember the knowledge learned from previous data. Therefore, the integration
  and interaction between these two components form a unified framework (APT), in
  which graph pre-training is performed in a progressive and iterative way. Experiment
  results show that the proposed APT is able to obtain an efficient pre-training model
  with fewer training data and better downstream performance.'
codebase_url: https://github.com/galina0217/APT
...
- &id004
  ...
  - model: No pretrain
    ...
    protocol_note: ROC-AUC on Bace with scaffold splitting
    date: Nov 2, 2023
  ...
  - model: MGSSL
    ...
    is_std_outlier: false
  - model: APT (Mole-BERT)
    model_key: apt (mole-bert)
    model_plain: APT (Mole-BERT)
    value: 0.818
    std: 0.014
    paper_value: 0.818
    paper_std: 0.014
  ...
  - model: Mole-BERT
    ...
    feature_source_evidence: Mole-BERT is a pre-trained model used as a backbone.
    table_ref: Table 10
    source_ref: xiamole
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: graph_classification
...
- &id005
  ...
  - model: No pretrain
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: ROC-AUC on BBBP with scaffold split
    date: Nov 2, 2023
    date_display: Nov 2023
    date_iso: '2023-11-02'
    published_venue: NeurIPS 2023
  ...
  - model: GraphCL
    ...
    value_gap_source_date_iso: '2022-06-02'
    value_gap_source_date_label: KDD 2022
    gap_vs_at_pub: 0.20899999999999996
    worse_than_at_pub: true
    surpassed_since_pub: false
    better_than_at_pub: false
    insignificant_improvement_at_pub: false
  ...
  - model: Mole-BERT
    ...
    uses_external_data: 1
    input_feature_source: llm_features
    feature_source_evidence: Mole-BERT is a backbone model
    table_ref: Table 10
    source_ref: xiamole
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: graph_classification
  ...
  - model: AD-GCL
    ...
    comparison_source_title: Graph Self-Contrast Representation Learning
    comparison_source_arxiv: '2309.02304'
    is_best: false
    is_std_outlier: false
  - model: G-Contextual
    model_key: g-contextual
...
- &id001
  ...
  - model: APT (Mole-BERT)
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: graph classification task using micro F1 on COLLAB dataset
    date: Nov 2, 2023
    date_display: Nov 2023
    date_iso: '2023-11-02'
    published_venue: NeurIPS 2023
    ...
    is_std_outlier: false
  - model: GCC (fine-tune)
    model_key: gcc (fine-tune)
    model_plain: GCC (fine-tune)
    value: 0.811
    std: null
    paper_value: 0.811
    paper_std: null
...
- &id006
  ...
  - model: No pretrain
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: ROC-AUC on ClinTox with scaffold splitting
    date: Nov 2, 2023
    date_display: Nov 2023
    date_iso: '2023-11-02'
    published_venue: NeurIPS 2023
  ...
  - model: Mole-BERT
    ...
    uses_external_data: 1
    input_feature_source: llm_features
    feature_source_evidence: Mole-BERT is a pre-trained model
    table_ref: Table 10
    source_ref: xiamole
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: graph_classification
  ...
  - model: SimGRACE
    ...
    value_gap_source_date_iso: '2024-06-01'
    value_gap_source_date_label: '2024'
    gap_vs_at_pub: 0.11340000000000006
    worse_than_at_pub: false
    surpassed_since_pub: false
    better_than_at_pub: true
    insignificant_improvement_at_pub: false
...
- &id002
  ...
  - model: APT-P (fine-tune)
    model_key: apt-p (fine-tune)
    model_plain: APT-P (fine-tune)
    value: 0.767
    std: 0.0101
    paper_value: 0.767
    paper_std: 0.0101
    ...
    uses_external_data: 1
    input_feature_source: raw_features
    feature_source_evidence: pre-training on unlabeled data
    table_ref: Table 2
    source_ref: this paper
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: Micro F1 on imdb-binary test set using fine-tuning
    date: Nov 2, 2023
    date_display: Nov 2023
    date_iso: '2023-11-02'
    published_venue: NeurIPS 2023
    ...
    is_best: true
    is_std_outlier: false
  - model: APT-R (fine-tune)
    model_key: apt-r (fine-tune)
    model_plain: APT-R (fine-tune)
    value: 0.766
    std: 0.0102
    paper_value: 0.766
    paper_std: 0.0102
    ...
    is_best: true
    is_std_outlier: false
  - model: APT-G (fine-tune)
    model_key: apt-g (fine-tune)
    model_plain: APT-G (fine-tune)
    value: 0.7629
    std: 0.0089
    paper_value: 0.7629
    paper_std: 0.0089
  ...
  - model: GCC (fine-tune)
    ...
    is_best: true
    is_std_outlier: false
  - model: APT-L2 (fine-tune)
    model_key: apt-l2 (fine-tune)
    model_plain: APT-L2 (fine-tune)
    value: 0.7593
    std: 0.0084
    paper_value: 0.7593
    paper_std: 0.0084
  ...
  - model: GIN
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: Micro F1 on imdb-binary test set
    date: Nov 2, 2023
    date_display: Nov 2023
    date_iso: '2023-11-02'
    published_venue: NeurIPS 2023
  ...
  - model: DGCNN
    ...
    is_best: false
    is_std_outlier: false
  - model: JOAO (fine-tune)
    model_key: joao (fine-tune)
    model_plain: JOAO (fine-tune)
    value: 0.685
    std: 0.0361
    paper_value: 0.685
    paper_std: 0.0361
    ...
    is_best: false
    is_std_outlier: false
  - model: GraphCL (fine-tune)
    model_key: graphcl (fine-tune)
    model_plain: GraphCL (fine-tune)
    value: 0.669
    std: 0.0439
    paper_value: 0.669
    paper_std: 0.0439
...
- &id003
  ...
  - model: JOAO
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: ROC-AUC on Tox21 with scaffold splitting
    date: Nov 2, 2023
    date_display: Nov 2023
    date_iso: '2023-11-02'
    published_venue: NeurIPS 2023
  ...
  - model: Mole-BERT
    ...
    uses_external_data: 1
    input_feature_source: llm_features
    feature_source_evidence: Mole-BERT is a language model based pre-training
    table_ref: Table 10
    source_ref: xiamole
    variant_inference_reason: 'dataset: exact match'
    evaluation_task: graph_classification
...
- &id008
  ...
  - model: 3D Infomax
    ...
    evaluation_task: graph_classification
    protocol_decision: standard
    protocol_note: ROC-AUC on ToxCast with scaffold splitting
    date: Nov 2, 2023
    date_display: Nov 2023
    date_iso: '2023-11-02'
    published_venue: NeurIPS 2023
...
main_figure: /figures/2311.01038/main_figure.jpegoptim.jpg
---
